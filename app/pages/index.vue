...
function buscar(emailRetornar) {
  ...
  for (let i = 0; i < arrayUsuario.value.length; i++) {
    let usuarioEncontrado = arrayUsuario.value[i];
...
<template>
  <div class="flex flex-col gap-22 justify-center items-center">
    <h1 id="hero">Homechat | O lar da conversa</h1>
    <div id="divForm" class="flex items-center justify-center w-md p-5 rounded-xl">
      <UForm :validate="verificar" :state="usuario" class="flex flex-col space-y-4 w-full" @submit="redirecionar">
